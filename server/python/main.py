...
def create_hand(body: HandRequest):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        cursor = conn.execute(
            """
            INSERT INTO hands (hand_number_1, hand_number_2, hand_sute_1, hand_sute_2, created_at)
            VALUES (?, ?, ?, ?, ?)
            """,
            (body.cards[0].rank, body.cards[1].rank, body.cards[0].suit, body.cards[1].suit, created_at)
        )
...
def get_hands():
    ...
    with sqlite3.connect(DB_PATH) as conn:
        rows = conn.execute(
            """
            SELECT id, hand_number_1, hand_number_2, hand_sute_1, hand_sute_2, created_at
            FROM hands
            ORDER BY id DESC
            """
        ).fetchall()
    ...
    result = []
    for row in rows:
        cards = [Card(rank=row[1], suit=row[3]), Card(rank=row[2], suit=row[4])]
        result.append(HandResponse(id=row[0], cards=cards, created_at=row[5]))
